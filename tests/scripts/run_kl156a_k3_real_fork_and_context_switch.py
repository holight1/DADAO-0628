#!/usr/bin/env python3
"""KL-156a: real fork/context switch (copy_thread/__switch_to/ret_from_fork).

KL-154a/155a frozen the diagnosis that `kernel_thread()` always returned
-ENOSYS because arch/dadao's `copy_thread()`/`__switch_to()` were fail-closed
placeholders, so kernel_init (and every later kernel thread) was never
actually created or executed. This gate:

  1. verifies the frozen KL-154a/KL-155a root, Linux, QEMU and LLVM
     identities and evidence are unchanged;
  2. verifies the one new Linux commit this task adds (switch.S's
     dadao_switch_to_asm/dadao_ret_from_fork, process.c's real
     __switch_to/copy_thread, the CONFIG_DADAO_M1_PROGRESS markers) and
     its exported patch/patch-id/patch-series bare-pin replay;
  3. rebuilds a fresh KCFLAGS=-O0 Image and boots it (under `-icount
     shift=0`, the KL-155a-mandated QEMU invocation), reading a 27-word
     oracle via QMP, confirming:
       - rest_init_pid is a real pid (1), not -ENOSYS;
       - kernel_init_enter fires (the new task was actually scheduled);
       - the idle->kernel_init->idle round trip completes
         (SWITCH_FROM_IDLE + SWITCH_TO_IDLE + IDLE_ENTER);
       - SWITCH_COUNT grows (1000+ genuine context switches);
       - boot reaches the next honest wall (ROOT_MOUNT_ATTEMPT + the VFS
         root-mount panic in the console);
  4. reads back init_task's saved switch frame from guest memory and
     verifies its structural invariants (K2-style key-value check);
...
EVIDENCE = EVIDENCE_PARENT / "kl156a-real-fork-and-context-switch"
EVIDENCE_LOCK = EVIDENCE_PARENT / ".kl156a-real-fork-and-context-switch.lock"
EVIDENCE_CURRENT = (
    EVIDENCE_PARENT / ".kl156a-real-fork-and-context-switch.current.json"
)
LINUX_SERIES = ROOT / "components" / "linux" / "patches" / "series"
MANIFEST = ROOT / "manifests" / "components.lock.toml"

KL155_SUMMARY = ROOT / ".work" / "evidence" / "kl155a-real-cfx-trap-vector-and-timer-clockevent" / "summary.json"
KL155_CURRENT = ROOT / ".work" / "evidence" / ".kl155a-real-cfx-trap-vector-and-timer-clockevent.current.json"
KL155_SUMMARY_SHA256 = "260e0547fff62371c2cb8594c118fecbe945d8d5702848e8591644cd061ff6be"
...
LINUX_HEAD_BEFORE = "b69106ec3b80cca22990857fa9ac907e8ddd4746"
LINUX_FIX_COMMIT = "78601a1d2996788d1036b09a55e62784e523d89b"
LINUX_FIX_PATCH_NAME = "0034-dadao-add-KL-156a-real-fork-and-context-switch.patch"
LINUX_SERIES_COUNT = 34

RAM_BASE = 0x80000000
SCRATCH_BASE = 0x87FD0000
ORACLE_WORDS = 27
ORACLE_SIZE = ORACLE_WORDS * 8
# Word indexes within the 27-word oracle.
...
def acquire_exclusive_lock() -> int:
    ...
    try:
        ...
        os.close(descriptor)
        raise GateError(
            f"another KL-156a runner owns exclusive lock {EVIDENCE_LOCK}"
        ) from exc
...
ROOT_KL155A_FIX_COMMIT = "d92a5e6e15b6f6ec3d37674efa3cdf204aa34561"


def verify_root_frozen_identity() -> dict[str, object]:
    head = execute("root-head", ["git", "-C", str(ROOT), "rev-parse", "HEAD"]).stdout.strip()
    expected = execute(
        "root-head-expected", ["git", "-C", str(ROOT), "rev-parse", ROOT_KL155A_FIX_COMMIT]
    ).stdout.strip()
    if head != expected:
    ...
    allowed_prefixes = (
        ...
        "?? code-agent/tasks/KL-156a-",
        "?? components/linux/patches/0034-",
        "?? tests/scripts/run_kl156a_",
...
def verify_kl155a_frozen_evidence() -> dict[str, object]:
    if not KL155_SUMMARY.is_file():
        raise GateError(f"KL-155a summary missing: {KL155_SUMMARY}")
    if sha256(KL155_SUMMARY) != KL155_SUMMARY_SHA256:
        raise GateError("KL-155a frozen summary identity drift")
    summary = json.loads(KL155_SUMMARY.read_text())
    required = {
        ("task",): "KL-155a",
        ("result",): "PASS",
        ("counts", "pass"): 3,
        ...
        ("marker_analysis", "last_marker_reached"): "idle_enter",
        ("marker_analysis", "rest_init_pid_signed"): -38,
    }
    for keys, expected in required.items():
        ...
        if value != expected:
            raise GateError(f"KL-155a evidence drift at {'.'.join(keys)}: {value!r} != {expected!r}")
    if not KL155_CURRENT.is_file():
        raise GateError("KL-155a external current-state file missing")
    current = json.loads(KL155_CURRENT.read_text())
    if current.get("state") != "PASS" or current.get("valid_pass") is not True:
        raise GateError("KL-155a external current-state is not a valid PASS")
    return {
        "path": str(KL155_SUMMARY),
        "sha256": sha256(KL155_SUMMARY),
...
def main() -> int:
    ...
    kl155a = verify_kl155a_frozen_evidence()
    ...
    if not positive["console_has_vfs_panic"]:
        raise GateError("positive console does not contain the VFS root-mount panic (next wall)")
    ...
    summary = {
        ...
        "diagnosis": {
            "result": (
                "kernel_thread(kernel_init) returns a real pid (1); kernel_init "
                "runs its whole body; the real scheduler executes 1000+ genuine "
                "context switches (kthreadd + workqueue machinery); the "
                "idle->kernel_init->idle round trip completes with the boot/idle "
                "context's saved frame intact (K2-style readback). Next honest "
                "wall: the VFS root-mount panic (prepare_namespace on a machine "
                "with no root filesystem). This does NOT claim full boot to "
                "userspace/login."
            ),
        },
        "runner_identity": runner_identity,
    ...
    if current_runner_identity != runner_identity:
        raise GateError("runner changed while KL-156a gate was executing")
    summary["artifact_manifest"] = publish_artifact_manifest(runner_identity)
    finalize_success(summary)
    print(json.dumps(summary, indent=2, sort_keys=True))
    print("PASS: KL-156a K3 real fork + context switch (3/3, FAIL=0, SKIP=0)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(exc, GateError):
            raise SystemExit(f"KL-156a FAIL: {exc}") from exc
        raise
    finally:
