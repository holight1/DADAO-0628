...
`docs/proposals/cfx-mem-cfx-mask.md`（现为 v2）。**独立评审（WP-001a，
`docs/proposals/cfx-mem-cfx-mask-review.md`）已完成**：核心缺口确认
真实存在（范围收窄为"内部存储块"，不含寄存器组），但 v1 给出的具体
插入点技术上不完整（改了路由伪代码却没同步改"何时判定为非法"这条
触发条件，按原文实际不会生效）——已按评审意见修订为 v2（不再新写
路由逻辑，只在触发条件里加一条新判据，交给既有路由伪代码处理）。
**当前状态：草案 v2，未提交、未被 wiki 团队 review、K1-K3 现有实现
完全不受影响**——记在这里同样是作为未来对接 wiki 团队时的候选项，
不代表已被采纳。
