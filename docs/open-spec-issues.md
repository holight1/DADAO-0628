...
| Varargs | DL-072a implements the wiki-defined caller-populated, declaration-order, unified 8-byte save area for scalar RD/RB values; the former RD-only callee spill and concrete pointer-loss issue are closed (`docs/issues-archive.yaml`). `va_arg` now advances 8 and big-endian narrow loads right-adjust within the slot. Still open: RF varargs, full aggregate coverage, and a wiki wording conflict—“save-area base = incoming SP” cannot simultaneously place all ordinary overflow copies below locals and the save area at higher addresses without extra base metadata. DL-072a preserves the normative incoming-SP/`va_start` formula and separately tests fixed and unnamed overflow behavior; wiki clarification is still required before claiming the full RF/aggregate ABI complete. | Complete ABI |
