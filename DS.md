...
> **⚠ AC/零 finding 也必须写回记录（ML-002b/002c 数据点：DS 做了自审但判 Accepted 就没记录 → 与"跳审"外观等同、架构师无法区分 → 一律按跳审打回）。**「subagent 说没问题」不等于「无事可写」——**AC 恰恰要留证据证明审过了**。零 finding 时，把下面最小模板填进 `## 审阅记录（subagent）` 区（替换占位）：
> ```
> ### 审阅记录（subagent · 判决 = 通过）
> - subagent 已读 reviewer.md + 逐行审 diff（改动文件：xxx）
> - 核验点：<任务特别核的每一项，逐条 ✓，附一句证据>（如 trap 编码 op=0x76 ✓ / gem5 write grep -c=1 ✓ / QEMU==gem5 ✓）
> - 未测输入/边界推敲：<审了哪些未测情形，结论>
> - finding：无（判决=通过）
> ```
> **占位没被替换成上述实质记录 = 未自审。** 无论 AC 与否，占位区留原样一律打回。
