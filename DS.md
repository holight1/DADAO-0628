...
2. **DS 开一个 subagent** 做 review：subagent 先读 `reviewer.md`，然后**独立重跑 ground-truth**（重 build、重跑测试/差分/llc 取 MIR 等，**亲跑不读、不采信完成区**），核退出码、核不回归、查凑绿（`|| true`/删工作文件/伪造输出）。
3. subagent 把 **review 意见 + 发现的问题 + 判决** 写进任务 md 的 **`## 审阅记录（subagent）`** 区。
4. DS 据 review **修复**，把**修复情况**也追加到该区；修完再自审一轮直到通过。
5. DS 返回架构师（此时交回的已是自审修过的版本）。架构师做最终独立复跑验收后 commit。
